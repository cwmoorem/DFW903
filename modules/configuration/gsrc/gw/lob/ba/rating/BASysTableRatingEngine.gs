    ...
    if ( lineVersion.Branch.RefundCalcMethod == TC_SHORTRATE ) { // context.CancelMethod returns null if the period is not canceled
      ...
      for ( st in basesMap.Keys ) {
        ...
        if ( subtotal != 0 ) {
          ...
          var termSubTotal =  getCostsUpToRatedOrder( BARatedOrderType.TC_CANCELSHORTRATEPENALTY ).where( \ cost -> cost.ProrationMethod != TC_FLAT).where( \ c -> c.JurisdictionArg.State == st ).sum(\ c -> p.prorate(lineVersion.Branch.PeriodStart, p.findEndOfRatedTerm(lineVersion.Branch.PeriodStart, c.NumDaysInRatedTerm),
