    ...
    switch (covPart) {
      case TC_IMSIGNPART : 
        if (this.IMSignPart == null) {
          ...
          if (!getAllExistingCoverageParts(TC_IMSIGNPART).HasElements) {
            initializeSignAutoNumber = true 
          } else {
            var existingSignPart = getAllExistingCoverageParts(TC_IMSIGNPART).first() as IMSignPart
      ...
      case TC_CONTRACTORSEQUIPPART : 
        if (this.ContractorsEquipPart == null) {
          ...
          var existingAutoNumberSequence : AutoNumberSequence
          if (!getAllExistingCoverageParts(TC_CONTRACTORSEQUIPPART).HasElements) {
            initializeAutoNumberSequence = true 
          } else {
            var existingContractorsEquipPart = getAllExistingCoverageParts(TC_CONTRACTORSEQUIPPART).first() as ContractorsEquipPart
      ...
      case TC_IMACCOUNTSRECPART :
        if(this.IMAccountsRecPart == null) {
          ...
          if (!getAllExistingCoverageParts(TC_IMACCOUNTSRECPART).HasElements) {
            initializeAutoNumberSequence = true 
          } else {
            var existingAccountRecPart = getAllExistingCoverageParts(TC_IMACCOUNTSRECPART).first() as IMAccountsRecPart
