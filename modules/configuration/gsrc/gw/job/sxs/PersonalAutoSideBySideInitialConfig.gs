  ...
  override function configureInitialPolicies(policyPeriods : PolicyPeriod[]) {
    ...
    if (null==policyPeriods[0].Offering) {
      policyPeriods.eachWithIndex(\pp,i -> {
        switch(i) {
          case 0:
            editIfQuotedSetOfferingAndSync(pp,PCCoercions.makeProductModel<Offering>("PABasic"))
            break
          case 1:
            editIfQuotedSetOfferingAndSync(pp,PCCoercions.makeProductModel<Offering>("StandardProgram"))
            break
          case 2:
            editIfQuotedSetOfferingAndSync(pp,PCCoercions.makeProductModel<Offering>("PAPremium"))
            break
        }
      })
    }
