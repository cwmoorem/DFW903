...
class QuoteProcess {
  ...
  function requestQuote(jobWizardHelper : JobWizardHelper, valLevel : ValidationLevel, ratingStyle : RatingStyle, warningsThrowException : boolean) {
    ...
    try{
      ...
      if (jobWizardHelper != null) { //null check to account for invocations thru batch process
        jobWizardHelper.Wizard.EditController.commitChanges(BundleValidationOption.validateErrorsAndWarnings())
      ...
      PCProfilerTag.QUOTE_FINISH.execute( \-> {
        try {
  ...
  private function copyDiffPropertiesFromFutureSlices() : List<DiffItem> {
    ...
    partitionedEffDatedProperties.eachKey( \ fixedID -> {
      ...
      diffPropertiesWithSameFixedID
          .where( \ diffProperty -> {
            return diffPropertyMergedAndInFutureSlice(diffProperty) and not diffPropertyExistsOnEditEffectiveDate(diffProperty, diffPropertiesWithSameFixedID)
          })
  ...
  private function diffPropertyMergedAndInFutureSlice(diffProperty: DiffProperty) : boolean {
    return EffDatedChangeType.TC_MERGE == diffProperty.EffDatedBean.ChangeType and
        _branch.EditEffectiveDate.before(diffProperty.EffDatedBean.EffectiveDate)
  }
  /**
  ...
  private function createDiffPropertyOnEditEffectiveDate(diffProperty : DiffProperty) : DiffProperty {
    ...
    var effDatedBean = diffProperty.EffDatedBean.VersionListUntyped?.getVersionAsOf(_branch.EditEffectiveDate)?.getSliceUntyped(_branch.EditEffectiveDate) as EffDated
    var isNewlySlicedBean = effDatedBean?.EffectiveDate == _branch.EditEffectiveDate
    if (isNewlySlicedBean) {
      ...
      if (not newDiffProperty.OOSChange) {
        newDiffProperty = null
      }
    }
  ...
  function attemptQuoteReleaseForNonprivilegedUser() {
    ...
    var grants = currentUser.UserAuthorityProfiles*.UWAuthorityProfile*.Grants
