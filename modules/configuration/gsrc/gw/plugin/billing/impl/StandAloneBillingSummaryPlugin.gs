...
class StandAloneBillingSummaryPlugin implements IBillingSummaryPlugin {
  ...
  protected function createMockPolicyPeriodBilling(policyNumber : String, termNumber : int,
                                                   withoutInvoiceStream : Boolean = false): BillingPeriodInfo{
  ...
  private function findPolicyTermInfos(policyNumber : String, termNumber : int) : BillingTermInfo[] {
    ...
    return query
        ...
        .transformQueryRow(\ row -> {
          return new PolicyTermInfo(row.getColumn(POLICY_NUMBER) as String, row.getColumn(TERM_NUMBER) as Integer,
