...
class ActivitySearchCriteria extends EntitySearchCriteria<Activity> {
  ...
  function makeQuery() : Query<Activity> {
    ...
    if (AccountNumber != null && PolicyNumber != null) {
      ...
      policyPeriodTable.join("Policy").join("Account").compare(Account#AccountNumber, Equals, AccountNumber)
      ...
      query.join("Account").compare(Account#AccountNumber, Equals, AccountNumber)
    }
    return query
  }
