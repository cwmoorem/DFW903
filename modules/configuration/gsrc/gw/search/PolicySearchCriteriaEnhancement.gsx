...
enhancement PolicySearchCriteriaEnhancement : entity.PolicySearchCriteria {
  ...
  function search() : IQueryBeanResult<PolicyPeriodSummary> {
    ...
    // a matching VIN
    var subQuery = Query.make(PolicyPeriodSummary)
    ...
    */
    ...
    // the select as above.
    if (!meetsMinimumSearchCriteria()) throw new DisplayableException(DisplayKey.get("Web.Policy.MinimumSearchCriteria"));
  ...
  */
  ...
  private property get IsAuditSearch() : boolean {
    return new SearchObjectType[]{TC_FINALAUDIT, TC_PREMIUMREPORT}.contains(this.SearchObjectType)
  }
