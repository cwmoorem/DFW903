...
class PolicyPeriodBillingInstructionsManager implements BillingMethodState, PaymentPlansStateContainer, InvoiceStreamStateContainer, UpFrontPaymentStateContainer {
  ...
  static function isEditable(period : PolicyPeriod) : boolean {
    return period.ValidQuote and (
        period.Status == TC_NEW or period.Status == TC_DRAFT or period.Status == TC_QUOTED)
  }
  ...
  final function populateFromPolicyPeriod(period : PolicyPeriod) {
    ...
    try {
      ...
      if (!isEditable(period) or
          period.BillingMethod == null or
          !period.JobProcess.canViewQuote()) {
  ...
  private function getDownPaymentWithTax() : MonetaryAmount {
    ...
    if (downPayment != null) {
      var tax = _period.AllCosts.TaxSurcharges.AmountSum(PreferredSettlementCurrency)
      downPayment += tax
    }
    return downPayment
  }
  ...
  **/
  ...
  function setFieldsFromNewReportingPlan() {
    var auditSchedulePattern = AuditSchedulePatternLookup.selectByCode(ReportingPlanChoice.ReportingPatternCode)
  ...
  property get AuditSchedulePattern() : AuditSchedulePattern {
    return AuditSchedulePatternLookup.selectByCode(ReportingPlanChoice.ReportingPatternCode)
  }
