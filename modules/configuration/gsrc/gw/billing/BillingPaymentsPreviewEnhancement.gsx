...
enhancement BillingPaymentsPreviewEnhancement : BillingPaymentsPreview {
  ...
  */
  ...
  function getTypeAmount(invoiceItemType : String, policyPeriod : PolicyPeriod) : MonetaryAmount {
    return maxOf(policyPeriod.PreferredSettlementCurrency,
                 this.PreviewItems
                     .where(\p -> p.Type == invoiceItemType))
  }
  ...
  private function maxOf(currency : Currency, previewItems : PaymentPreviewItem[]) : MonetaryAmount {
    return previewItems.IsEmpty
        ? 0bd.ofCurrency(currency)
        : previewItems.max(\item -> item.Amount)
  }

  private function getOneTimeCharge(currency : Currency, previewItems : PaymentPreviewItem[]) : MonetaryAmount {
    return previewItems.IsEmpty
        ? 0bd.ofCurrency(currency)
        // Return a one-time fee (appears only once) what implies it should have a frecuency of 1
        : getChargeByFrecuencyCheck(currency, previewItems, \value -> value == 1)
  }

  private function moreThanOneTimeOf(currency : Currency, previewItems : PaymentPreviewItem[]) : MonetaryAmount {
    return previewItems.IsEmpty
        ? 0bd.ofCurrency(currency)
        // Return a representative fee (appears more than once) what implies it should have a frecuency higher than 1
        : getChargeByFrecuencyCheck(currency, previewItems, \value -> value > 1)
  }

  private function getChargeByFrecuencyCheck(currency : Currency, previewItems : PaymentPreviewItem[], frequency(value : Integer) : boolean) : MonetaryAmount {
