...
class InvoiceStreamStateContainerImpl implements InvoiceStreamStateContainer {
  ...
  // to preserve changes made to the invoice stream state, we keep the custom instance around
  // we could throw it away to reset after the user unselects custom billing
  var _customInvoiceStreamState : CustomBillingInvoiceStreamState
  ...
  construct(container : PolicyPeriodBillingInstructionsManager) {
    _invoiceStreamState = AbstractInvoiceStreamState.createDefaultBillingInvoiceStreamState(this)
    ...
    _customInvoiceStreamState = AbstractInvoiceStreamState.createCustomBillingInvoiceStreamState(this)
  }

  final function populateFromPolicyPeriod(period : PolicyPeriod) {
    CustomBilling = period.CustomBilling
    _invoiceStreamState.populateFromPolicyPeriod(period)
  }

  protected function selectCustomBilling(isCustom : boolean) {
    updateInvoiceStreamState(isCustom)
    resetInvoiceStreamsAndPaymentInstruments()
  }

  private function updateInvoiceStreamState(isCustom : boolean) {
    if (isCustom) {
      _invoiceStreamState = _customInvoiceStreamState
      _invoiceStreamState.clearInvoiceStreamState()
    } else {
      _invoiceStreamState = AbstractInvoiceStreamState.createDefaultBillingInvoiceStreamState(this)
    }
  }
