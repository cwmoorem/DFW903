...
class BillingPaymentsPreviewTrees {
  ...
  static function createBillingPaymentsPreviewTree(billingPaymentsPreview : BillingPaymentsPreview, showInvoices : boolean, expandCollapseAll : boolean) : TreeNode {
    var rootTree = new SimpleTreeNode(new BillingPaymentsPreviewTreeNode(null, null, null), false)
    var itemParents = new LinkedHashSet<Date>()

    for (item in billingPaymentsPreview.PreviewItems) {
      itemParents.add(item.DueDate)
    }
    ...
    for (parentDate in itemParents) {
      var parent = new BillingPaymentsPreviewTreeNode(parentDate, null, null)
  ...
  static private function createTreeNode(item: PaymentPreviewItem, type: ItemType, parentTree: TreeNode) : PaymentPreviewItem {
    var itemTreeNode = new BillingPaymentsPreviewTreeNode(null, item.TypeDisplay, item.ChargeCategoryDisplay)
