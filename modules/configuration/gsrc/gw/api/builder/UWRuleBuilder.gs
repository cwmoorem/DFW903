...
class UWRuleBuilder extends RuleBuilder<UWRule, UWRuleBuilder> {
  ...
  construct (excludeHead : boolean) {
    ...
    withName("UWIsssue " + id)
    withDescription("UWIsssue Description " + id)
    ...
    if (not excludeHead) {
      addPopulator(Integer.MAX_VALUE, \bean -> {
        _head = BizRulesDependencies.BizRulesVersionController.createNewHead(bean.Bundle, bean as UWRule, false) as UWRuleHead
      })
    }
  }
  ...
  property get LastCreatedRuleHead(): UWRuleHead {
    return _head
  }
