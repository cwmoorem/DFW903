...
uses gw.entity.ICancellationExtensions
uses gw.entity.IContactExtensions
uses gw.entity.ICostExtensions
uses gw.entity.ICoverageExtensions
uses gw.entity.IPolicyLocationExtensions
uses gw.entity.IPolicyPeriodExtensions
uses gw.entity.IRenewalExtensions
uses gw.entity.IUWIssueExtensions
...
uses gw.rating.rtm.validation.IRateTableDefinitionValidationFactory
...
class PCExtensionInterfacesMapper implements ExtensionInterfacesMapper {
  ...
  override function bindImplementations(config: ExtensionInterfacesConfig) {
    ...
    config.setImplementation<gw.contactmapper.ab800.IPCNameMapperExtensions>(new gw.contactmapper.ab800.PCNameMapperExtensionsImpl()) // 800 version
    ...
    config.setImplementation<IRateTableDefinitionValidationFactory>(\ valContext, definition -> new RateTableDefinitionValidation(valContext, definition))
    ...
    config.setImplementation<IQuoteHaltedExceptionFactory>(\ issues: List<String> -> new QuoteHaltedException(issues))
    ...
    config.setImplementation<gw.webservice.pc.pc800.contact.IAddressServiceExtensions>(\ -> gw.webservice.pc.pc800.contact.AddressService.Instance) // 800 version
    ...
    config.setImplementation<gw.webservice.pc.pc800.contact.IAddressDataExtensions>(\ address -> gw.webservice.pc.pc800.contact.AddressData.of(address)) // 800 version
    ...
    config.setImplementation<gw.webservice.pc.pc800.contact.IAddressDataCopierFactory>(\ addressData, shouldCopyNulls -> new gw.webservice.pc.pc800.contact.AddressDataCopier(addressData as gw.webservice.pc.pc800.contact.AddressData, shouldCopyNulls)) // 800 version
  ...
  private static class PolicyPeriodExtensionsImpl implements IPolicyPeriodExtensions {
    ...
    override function editIfQuoted(policyPeriod: PolicyPeriod) {
  ...
  private static class RateTableCellExtensionsImpl implements IRateTableCellExtensions {
    ...
    override function getValueRange<T extends Comparable>(rateTableCell: RateTableCell<T>, vp: RateTableCellValueProvider): List<T> {
