...
enhancement DesktopJobFiltersEnhancement : gw.api.web.desktop.DesktopJobFilters {
  ...
  static function producerFilters()  : BeanBasedQueryFilter[] {
    var openJobFilter = new NamedFilter(DisplayKey.get("Java.Desktop.ToolbarFilter.Jobs.Open"), Job.restrictors.open().asFilter())
    var createdJobsWithin7Days = new NamedFilter(DisplayKey.get("Java.Desktop.ToolbarFilter.Jobs.Created7Days"),
                  Job.restrictors.createdOnOrAfter(Date.Today.addDays(-7)).asFilter())
    var completedJobsWithin30Days = new NamedFilter(DisplayKey.get("Java.Desktop.ToolbarFilter.Jobs.Completed30Days"),
