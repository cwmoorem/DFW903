...
enhancement PolicyPeriodBaseEnhancement : PolicyPeriod {
  ...
  function getWizardWarningMessages() : List<String> {
    ...
    result.addAll(this.Policy.getContingencyWarningMessages())
  ...
  private function getProbablyObsoleteWarnings() : List<String> {
    if (not this.ValidQuote and this.Status == TC_DECLINED) {
      return {DisplayKey.get("Web.SubmissionWizard.Quote.SubmissionRejected", this.Job.DisplayStatus.toLowerCase())}
    } else if (not this.ValidQuote and this.Job.Complete) {
      return {DisplayKey.get("Web.SubmissionWizard.Quote.Completed", this.Status)}
    }
  ...
  */
  ...
  property get JobCompletionMessage() : String {
    switch(this.Status) {
      case TC_WITHDRAWN    : return DisplayKey.get("Web.JobComplete.Message.Withdrawn", this.Job.DisplayType, this.Job.JobNumber)
      case TC_DECLINED     : return DisplayKey.get("Web.JobComplete.Message.Declined", this.Job.DisplayType, this.Job.JobNumber)
      case TC_NOTTAKING    : return DisplayKey.get("Web.JobComplete.Message.NotTaking", this.Job.JobNumber)
      case TC_NOTTAKEN     : return DisplayKey.get("Web.JobComplete.Message.NotTaken", this.Job.DisplayType, this.Job.JobNumber)
      case TC_BINDING      : return DisplayKey.get("Web.JobComplete.Message.Binding", this.Job.DisplayType, this.Job.JobNumber)
      case TC_BOUND        : return DisplayKey.get("Web.JobComplete.Message.Bound", this.Job.DisplayType, this.Job.JobNumber)
      case TC_REINSTATING  : return DisplayKey.get("Web.JobComplete.Message.Reinstating", this.Job.DisplayType, this.Job.JobNumber)
      case TC_NONRENEWING  : return DisplayKey.get("Web.JobComplete.Message.NonRenewing", this.Job.JobNumber)
      case TC_NONRENEWED   : return DisplayKey.get("Web.JobComplete.Message.NonRenewed", this.Job.JobNumber)
      case TC_RENEWING     : return DisplayKey.get("Web.JobComplete.Message.Renewing", this.Job.JobNumber)
      case TC_RESCINDED    : return DisplayKey.get("Web.JobComplete.Message.Rescinded", this.Job.JobNumber)
      case TC_CANCELING    : return DisplayKey.get("Web.JobComplete.Message.ScheduledCancellation", this.Job.JobNumber, DateFormat.getDateTimeInstance().format(this.CancellationDate))
      case TC_AUDITCOMPLETE: return DisplayKey.get("Web.JobComplete.Message.AuditComplete", (this.Job as Audit).AuditInformation.AuditScheduleType, this.Job.JobNumber)
      case TC_WAIVED       : return DisplayKey.get("Web.JobComplete.Message.AuditWaived", this.Job.JobNumber)
      case TC_DRAFT        : return DisplayKey.get("Web.JobComplete.Message.Draft", this.Job.DisplayType, this.Job.JobNumber)
      case TC_QUOTED       : return DisplayKey.get("Web.JobComplete.Message.Quoted", this.Job.DisplayType, this.Job.JobNumber)
      default             : return DisplayKey.get("Web.JobComplete.Message.UnknownStatus", this.Status)
    }
  }
  ...
  property get JobCompletionTitle() : String {
    switch(this.Status) {
      case TC_REINSTATING : return DisplayKey.get("Web.JobComplete.Title.Reinstating", this.Job.DisplayType)
      case TC_CANCELING   : return DisplayKey.get("Web.JobComplete.Title.Canceling", this.Job.DisplayType)
      default            : return DisplayKey.get("Web.JobComplete.Title", this.Job.DisplayType, this.Status)
    }
  }
  ...
  function mustInvalidateQuoteBeforeReleasingUWEditLock(authorityProfiles : UWAuthorityProfile[]) : boolean {
    return this.ValidQuote and not this.UWIssuesActiveOnly.issuesBlockingUser(TC_BLOCKSQUOTERELEASE, authorityProfiles).IsEmpty
  }
  ...
  /**
   * Renders to a default filename a PolicyPeriod in a printable format using the given template
   * @param templateType the name of the template to use
   * @deprecated Use {@link #printDocument(String, Map)} instead
   */
  @Deprecated("Use printDocument(String, Map) instead")
  function printDocument(templateType : String) {
    printDocument(templateType, templateType)
  }

  /**
   * Renders to the given filename a PolicyPeriod in a printable format using the given template
   * @param templateType the name of the template to use
   * @param fileName the name of the file to generate
   * @deprecated Use {@link #printDocument(String, String, Map)} instead
   */
  @Deprecated("Use printDocument(String, String, Map) instead")
  function printDocument(templateType : String, fileName : String) {
    printDocument(templateType, fileName, DocumentsHelper.getPrintQuoteParameters())
  }
