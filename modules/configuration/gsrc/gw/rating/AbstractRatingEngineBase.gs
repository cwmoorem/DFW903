...
abstract class AbstractRatingEngineBase<SL> {
  ...
  protected function getStateTaxRate( st : Jurisdiction ) : BigDecimal {
    ...
    if (st == TC_CA) { rate = 0.0725 }
    else if (st == TC_KY) { rate = 0.06 }

    return rate
  }

  /**
   * A rating engine normally computes a Term Amount.  (In Cost and CostData, this will be the value for ActualTermAmount
