...
abstract class CostData<S extends entity.Cost, V extends PolicyLine> implements ICostData<S, V> {
  ...
  private class LazyFXConversion extends LockingLazyVar<MonetaryAmount> {
    ...
    function scaleWithLogging(logger: WorksheetLogger) : MonetaryAmount {
      ...
      var scaledAmt = logger.let(DisplayKey.get("Web.Policy.RatingWorksheet.ConvertedTermAmountRounded"),
          \ -> logger.Term.fn(gw.rating.flow.CalcRoutineRoundingMethod, "setScale", "Rounding",
              \ ->  gw.rating.flow.CalcRoutineRoundingMethod.setScale(  logger.arg("Value", \-> logger.Term.vr(DisplayKey.get("Web.Policy.RatingWorksheet.ConvertedTermAmount"), converted)),
                  logger.arg("Scale", \ ->typekey.RoundingScaleType.get(CurrencyUtil.getStorageScale(SettlementCurrency)+"")),
                  logger.arg("Mode", \ -> typekey.RoundingModeType.getRoundingModeTypeForRoundingMode(CurrencyUtil.RoundingMode)))))
   ...
   */
  private function initializeCostData(effDate : Date, expDate : Date, asRatedCurrency : Currency, rateCache : PolicyPeriodFXRateCache) {
