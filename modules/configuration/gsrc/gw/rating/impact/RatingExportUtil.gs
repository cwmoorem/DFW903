...
class RatingExportUtil {
  ...
  private static function addCosts(testCase : ImpactTestingTestCase, sheet : XSSFSheet, rowNumber : int, colToColIndex : Map<ImpactTestingWorksheetColumnType, Integer>,
   ...
   //populate, add to file
    ...
    var dataMap : Map<ImpactTestingWorksheetColumnType, Object> = {
      ...
      InvalidQuote -> ImpactTestingWorksheetColumnTypeDef.getLabel(InvalidQuote)
    }
  ...
  private static function doCostsForOnePeriod(baselinePeriod : PolicyPeriod, comparison : PolicyPeriod, addRow : block(m : Map<ImpactTestingWorksheetColumnType, Object>)) {
    if (not baselinePeriod.ValidQuote and not comparison.ValidQuote) {
      addRow({InvalidQuote -> DisplayKey.get("Web.Rating.ImpactTesting.NotQuotedBaselinePeriod")})
      return
    }
    ...
    orderedList.each(\ s -> {
      ...
      if (comparison.ValidQuote) {
        ...
        dataMap.put(InvalidQuote, DisplayKey.get("Web.Rating.ImpactTesting.NotQuotedTestPeriod"))
      }
      ...
      if (baselinePeriod.ValidQuote) {
        ...
        dataMap.put(InvalidQuote, DisplayKey.get("Web.Rating.ImpactTesting.NotQuotedBaselinePeriod"))
      }
