...
class CCProductModelCodeMappingUtil {
  ...
  public static function getCCTypeCodeForPMType(pmObject: ProductModelClass, warningStream: OutputStream = null) : String {
    ...
    if (GosuStringUtil.isBlank(pmObject.Code)) {
    ...
    if (genCode.length() > 50) {
      if (warningStream != null) {
        warningStream.write(StreamUtil.toBytes("Warning: Code exceeds max allowed 50 characters for a typecode: [" + pmObject.Code + "]. Code trimmed in order to map it.\n"))
      }
