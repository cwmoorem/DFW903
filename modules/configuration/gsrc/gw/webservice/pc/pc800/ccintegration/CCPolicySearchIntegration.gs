...
class CCPolicySearchIntegration {
  ...
  private function extractSearchCriteria(ccCriteria : CCPCSearchCriteria) : PolicySearchCriteria {
    ...
    criteria.NonRenewalCode = typekey.NonRenewalCode.get(ccCriteria.NonRenewalCode)
    criteria.PolicyNumber = ccCriteria.PolicyNumber
    criteria.PolicyStatus = typekey.PolicyPeriodStatus.get(ccCriteria.PolicyStatus)
    ...
    criteria.PrimaryInsuredState = typekey.State.get(ccCriteria.PrimaryInsuredState)
    criteria.PrimaryInsuredPostalCode = ccCriteria.PrimaryInsuredPostalCode
    criteria.PrimaryInsuredCountry = typekey.Country.get(ccCriteria.PrimaryInsuredCountry)
    // avoid setting the state criteria when primary insured state is unset
    if(ccCriteria.PrimaryInsuredState != null) {
      criteria.State = AddressJurisdictionHandler.getJurisdiction(typekey.State.get(ccCriteria.PrimaryInsuredState), typekey.Country.get(ccCriteria.PrimaryInsuredCountry))
    }
