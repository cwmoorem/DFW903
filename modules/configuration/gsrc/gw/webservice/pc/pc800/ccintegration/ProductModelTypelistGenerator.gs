...
class ProductModelTypelistGenerator {
  ...
  private function genCCProductModelXML() : XMLNode {
    ...
    for (lob in LOBProductMap.Keys) {
      ...
      var LOBNode = createTypeCodeNode(LOBCodeRoot, lob.PublicID, lob.Name, lob.Description)
      ...
      if (lobProductEntry != null) {
        for (prod in lobProductEntry) {
          createCategoryNode(LOBNode, "PolicyType", prod.PublicID)
        }
      }
      ...
      var lobLossEntry = (LOBLossTypeMap.get(trimTypeCode(lob.PublicID)) as ArrayList<String>)
    ...
    for (prod in gw.api.productmodel.ProductLookup.getAll() ) {
      ...
      var PolicyTypeNode = createTypeCodeNode(PolicyTypeRoot, prod.PublicID, prod.Name, prod.Description)
      var previousCoverageTypes = priorCoverageMap[prod.PublicID]
      
      // Add category elements for each LOB
      for (lob in prod.LinePatterns) {
        createCategoryNode(PolicyTypeNode, "LOBCode", lob.PublicID)
        ...
        for (cov in lob.CoverageCategories.flatMap(\ cat -> cat.CoveragePatterns)) {
          createCategoryNode(PolicyTypeNode, "CoverageType", cov.PublicID)
      ...
      for (policyNode in priorPolicyTypeXML.Children) {
        ...
        if (isMatchingTypecode(policyNode, trimTypeCode(prod.PublicID))) {
    ...
    for (lob in LOBProductMap.Keys) {
      for (cov in lob.CoverageCategories.flatMap(\ cat -> cat.CoveragePatterns) ) {
        ...
        var CovTypeNode = createTypeCodeNode(CoverageTypeRoot, cov.PublicID, cov.Name, cov.Description)
        ...
        for (prod in LOBProductMap.get(lob)) {
          createCategoryNode(CovTypeNode, "PolicyType", prod.PublicID)
        }
        ...
        for (covTerm in cov.CovTerms) {
          ...
          var CovTermNode = createTypeCodeNode(CovTermPatternRoot, covTerm.PublicID, covTerm.Name, covTerm.Description)
          ...
          createCategoryNode(CovTermNode, "CoverageType", cov.PublicID)
        ...
        var subtypeNodes = priorCovSubcovMap.get(trimTypeCode(cov.PublicID))
        ...
        if (subtypeNodes == null) {
          ...
          createCategoryNode(CovTypeNode, "CoverageSubtype", cov.PublicID)
            
          // Create a typelist node for the coverage subtype
          var subCovNode = createTypeCodeNode(CoverageSubtypeRoot, cov.PublicID, cov.Name, cov.Description)
          ...
          createCategoryNode(subCovNode, "CoverageType", cov.PublicID)
  ...
  private function localizeNameAndDescription(prodModel : ProductModelClass, typeListName : String,
    ...
    var ccTypeListDisplayKeyName = TypelistL10NPropertiesGateway.generateKey(true, typeListName, trimTypeCode(prodModel.PublicID))
    ...
    var ccTypeListDisplayKeyDescription = TypelistL10NPropertiesGateway.generateKey(false, typeListName, trimTypeCode(prodModel.PublicID))
