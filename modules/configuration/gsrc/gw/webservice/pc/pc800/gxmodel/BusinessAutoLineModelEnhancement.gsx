...
enhancement BusinessAutoLineModelEnhancement : gw.webservice.pc.pc800.gxmodel.businessautolinemodel.types.complex.BusinessAutoLine {
  ...
  function populate(line : BusinessAutoLine){
    ...
    for(v in this.Vehicles.Entry){
      ...
      for(cov in v.Coverages.Entry){
        vehicle.setCoverageExists(PCCoercions.makeProductModel<CoveragePattern>(cov.Pattern.Code), true)
        cov.$TypeInstance.populateCoverage(vehicle.getCoverage(PCCoercions.makeProductModel<CoveragePattern>(cov.Pattern.Code)))
      }
    }
    // populate drivers
  ...
  private function populateClause(j : BAJurisdiction, cov : gw.webservice.pc.pc800.gxmodel.clausemodel.types.complex.Clause){
    j.setCoverageConditionOrExclusionExists(PCCoercions.makeProductModel<ClausePattern>(cov.Pattern.Code), true)
    var c = j.getCoverageConditionOrExclusion(PCCoercions.makeProductModel<ClausePattern>(cov.Pattern.Code))
