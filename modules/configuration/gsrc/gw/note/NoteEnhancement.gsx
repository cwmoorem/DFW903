...
enhancement NoteEnhancement : Note {
  static function getLevelDisplayString(value : Object) : String {
    if(value typeis Account) {
      return DisplayKey.get("Java.Note.AccountLevelNote", value)
      ...
      return DisplayKey.get("Java.Note.PolicyLevelNote", period.PolicyNumberDisplayString)
    } else if (value typeis Job) {
      return getJobDisplayString(value)
    } else if (value typeis PolicyPeriod) {
      return DisplayKey.get("Java.Note.PolicyPeriodLevelNote", getJobDisplayString(value.Job), value.BranchName)
    } else if (value typeis Activity) {
      return DisplayKey.get("Web.NoteSearch.CurrentActivity", value)
    } else if (value typeis Contingency) {
      var contingency = value as Contingency
      return DisplayKey.get("Web.Contingency.RelatedTo", contingency.Title)
    } else {
      return DisplayKey.get("Note.UnknownLevel")
    }
  }
  ...
  private static function getJobDisplayString(job : Job) : String {
    ...
    return DisplayKey.get("Java.Note.JobLevelNote", job.DisplayType, job.JobNumber, dateOrStatus)
  }
