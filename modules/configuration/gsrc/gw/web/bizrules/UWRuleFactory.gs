...
@Export
class UWRuleFactory extends AbstractRuleFactory<UWRule, UWRuleHead> {

  construct() {
    super(UWRulesUtil.VersionController);
  }
  ...
  override function createRuleHead(ruleToClone: Optional<UWRule>): UWRuleHead {
    var head = super.createRuleHead(ruleToClone)
    if (ruleToClone.isPresent()) {
      var uwIssueType = head.HeadVersion.Rule.UWIssueType
