...
class AccountHolderPolicyMetrics {
  ...
  function firstEffectivePolicyDateFor(contact : Contact, roles : Set<Type<AccountContactRole>>) : Date {
    return policiesQueryFor(contact, roles).select({QuerySelectColumns.dbFunction(DBFunction.Min(Paths.make(Policy#OriginalEffectiveDate)))}).transformQueryRow(\row -> row.getColumn(0) as Date).single()
  }
  ...
  function cancellationsResultFor(contact: Contact, roles : Set<Type<AccountContactRole>>)
    ...
    queryPolicies.subselect("Account", CompareIn, queryAccounts, "ID")
  ...
  function policiesQueryFor(contact : Contact, roles : Set<Type<AccountContactRole>>)
    ...
    var queryAccounts = queryPolicies.join("Account")
    restrictAccountsByUserSecurity(queryAccounts, contact, roles)
  ...
  function calculateLifetimePremium(contact : Contact, roles : Set<Type<AccountContactRole>>, currency : Currency) : MonetaryAmountCalculationResult {
    ...
    activePoliciesQueryFor(contact, roles).select({
    ...
    activePolicyPeriodsQueryFor(contact, roles).select({
    ...
    override function filterNewQuery(cancellations : IQuery<Cancellation>) : IQuery<Cancellation> {
      ...
      if ( _cancelSource == TC_CARRIER ) {
        if ( _ignoredReasons == null ) {
          query.compare("CancelReasonCode", Equals, typekey.ReasonCode.TC_NONPAYMENT)
        } else {
   ...
   */
  private function restrictAccountsByUserSecurity(accounts : ISelectQueryBuilder,
   ...
   */
  private function restrictAccounts(accounts : ISelectQueryBuilder,
          contact : Contact, roles : Set<Type<AccountContactRole>>) {
   ...
   */
  function lookupAccountServiceTiersFor(contact : Contact, roles : Set<Type<AccountContactRole>>) : List<CustomerServiceTier> {
    var accounts = Query.make(Account)
        .compare(Account#ServiceTier, Relop.NotEquals, null)
    restrictAccountsByUserSecurity(accounts, contact, roles)
